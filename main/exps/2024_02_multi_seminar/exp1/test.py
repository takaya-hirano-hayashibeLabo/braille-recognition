...
def main():
    ...
    parser.add_argument("--frac_min",default=0.1,type=float)
    parser.add_argument("--frac_max",default=2.0,type=float)
    parser.add_argument("--frac_diff",default=0.1,type=float)
    ...
    with torch.no_grad():
        for frac in tqdm(frac_list):
            input_data_nrm,mean,std=transform(frac*input_data.reshape(n*t,c,h,w),data_size,mean,std)
