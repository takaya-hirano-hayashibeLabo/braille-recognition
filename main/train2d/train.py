...
class SNN(nn.Module):
    ...
    def forward(self,x):
        """
        :param x : [T x batch_size x channel x height x width]
...
def main():
    ...
    mean,std=np.mean(input_data),np.std(input_data)
    ...
    input_data_nrm,_,_=transform(input_data,(28,28))
    # >> データのリサイズと標準化 >>
    ...
    # out=net(input_data_nrm[:5])
    # print(out)
    # print(torch.Tensor(label_data[:5]).to(torch.int))
    # loss=criterion(out,torch.Tensor(label_data[:5]).to(torch.int))
    # exit(1)
